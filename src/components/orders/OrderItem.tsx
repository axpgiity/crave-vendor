...
export const OrderItem: React.FC<OrderItemProps> = ({ order, onUpdateStatus }) => {
    ...
    <li className="p-4 sm:p-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <span className="flex-shrink-0 text-sm font-medium text-gray-900">Order #{order.id}</span>
          ...
          <tbody className="divide-y divide-gray-200">
            {order.items.map((item, index) => (
              <tr key={index}>
                ...
                <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900 text-right">{item.quantity}</td>
              </tr>
            ))}
          </tbody>
        </table>
